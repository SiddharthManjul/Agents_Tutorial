...
export async function performRun(run: Run, thread: Thread, client: OpenAI) {
    
    while (run.status === "requires_action") {
        run = await handleRunToolCalls(run, thread, client);
    }
    ...
    return assistantMessage?.content[0] ||
        {type: "text", text: {value: "No response from assustant", annotations: []}};
